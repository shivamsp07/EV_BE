...
class Experiment():
    ...
    def run(self):
        ...
        if not os.path.exists('average_runs'):
            os.makedirs('average_runs')

        np.save('average_runs/' + id_run + '_reward_list.csv', self.reward_list)
        np.save('average_runs/' + id_run + '_average_list.csv', self.average_list)
        np.save('average_runs/' + id_run + '_Q.csv', self.Q)
        np.save('average_runs/' + id_run + '_PAR_list.csv', self.PAR_list)
        np.save('average_runs/' + id_run + '_max_list.csv', self.max_load_list)
        np.save('average_runs/' + id_run + '_Q_change_list.csv', self.Q_change_list)
        np.save('average_runs/' + id_run + '_Q_list.csv', self.Q_list)
    ...
    def plot_performance_metrics(id_run):
        reward_list = np.load(f'average_runs/{id_run}_reward_list.csv.npy')
        average_list = np.load(f'average_runs/{id_run}_average_list.csv.npy')
        PAR_list = np.load(f'average_runs/{id_run}_PAR_list.csv.npy')
        max_list = np.load(f'average_runs/{id_run}_max_list.csv.npy')

        fig, axs = plt.subplots(2, 2, figsize=(12, 8))

        axs[0, 0].plot(reward_list)
        axs[0, 0].set_title('Reward per Episode')
        axs[0, 0].set_xlabel('Episode')
        axs[0, 0].set_ylabel('Reward')

        axs[0, 1].plot(average_list)
        axs[0, 1].set_title('Average Grid Demand per Episode')
        axs[0, 1].set_xlabel('Episode')
        axs[0, 1].set_ylabel('Average Demand')

        axs[1, 0].plot(PAR_list)
        axs[1, 0].set_title('Peak-to-Average Ratio (PAR) per Episode')
        axs[1, 0].set_xlabel('Episode')
        axs[1, 0].set_ylabel('PAR')

        axs[1, 1].plot(max_list)
        axs[1, 1].set_title('Maximum Grid Demand per Episode')
        axs[1, 1].set_xlabel('Episode')
        axs[1, 1].set_ylabel('Maximum Demand')

        plt.tight_layout()
        plt.show()

    # Function to visualize the Q-value table
    def visualize_Q_table(id_run):
        Q = np.load(f'average_runs/{id_run}_Q.csv.npy')
        n_divisions_for_soc, n_hours, n_divisions_for_percent_honesty, _ = Q.shape

        for soc_bin in range(n_divisions_for_soc):
            for percent_honesty_bin in range(n_divisions_for_percent_honesty):
                fig, axs = plt.subplots(1, 1, figsize=(8, 6))
                Q_slice = Q[soc_bin, :, percent_honesty_bin, :]
                im = axs.imshow(Q_slice, cmap='viridis')
                axs.set_title(f'SOC Bin: {soc_bin}, Percent Honesty Bin: {percent_honesty_bin}')
                axs.set_xticks(np.arange(n_hours))
                axs.set_yticks([0, 1, 2])
                axs.set_yticklabels(['Charge', 'Discharge', 'Drive'])
                axs.set_xlabel('Hour')
    ...
    id_run = 'test.py'
    plot_performance_metrics(id_run)
    # visualize_Q_table(id_run)
